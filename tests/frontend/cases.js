...
group("שורת 'למה זה חשוב' בכרטיס");
{
  test('ההסבר מוצג כשיש אחד', () => {
    assert(X.has("why.className='news-why'"), 'צריך אלמנט ייעודי');
    assert(X.has('why.textContent=n.why;'), 'טקסט ולא HTML — אין הזרקה');
  });

  test('הכרטיס נראה כרגיל כשההסבר ריק', () => {
    // השרת מחזיר מחרוזת ריקה כשההסבר נפסל בוולידציה
    assert(X.has('if(n&&n.why){'), 'חייב להיות תנאי ולא הדפסה עיוורת');
    assert(X.has('content.append(title,meta);'), 'המסלול בלי הסבר נשמר');
  });

  test('ההסבר מובחן ויזואלית מהכותרת', () => {
    // הקורא חייב לדעת שזה טקסט שאנחנו כתבנו ולא חלק מהכותרת
    assert(X.has('.news-why{'), 'צריך עיצוב נפרד');
    assert(X.has('border-right:2px solid var(--line2)'), 'סימון ויזואלי');
  });
}

group('התדריך מבדיל בין מספר מהכתבה למספר שלנו');
{
  const B = X.load(['briefHtml', 'esc', 'seg', 'safeUrl'],
                   "var location={href:'https://stockiq.example/'};");

  test('קופסת המספרים נושאת תווית שאומרת שהם שלנו ועכשיו', () => {
    // נצפה בפועל: הכתבה אמרה "Okta עלתה ב-15%" והשורה שלנו הראתה 27.4%
    const h = B.briefHtml({
      what: 'Okta עלתה ב-15% אחרי הדוח.',
      impact: [{ ticker: 'OKTA', lines: ['המניה נסחרת ב-$171 — עלייה של 27.4% מאז הסגירה הקודמת.'] }],
      url: ''
    });
    assert(h.includes('OKTA'), 'הטיקר');
    assert(h.includes('הנתונים שלנו כרגע'), 'בלי זה שני המספרים נראים כמו סתירה');
  });

  test('התווית אינה מופיעה כשאין מספרים משלנו', () => {
    const h = B.briefHtml({ what: 'משהו קרה.', impact: [], url: '' });
