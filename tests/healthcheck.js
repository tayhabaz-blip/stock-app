...
async function checkAI() {
  ...
  // נצפה בפועל: "RSI של 37.4, המצביע על תנודתיות אפשרית". RSI אינו מדד
  // לתנודתיות — זו טעות מקצועית, לא ניסוח, ולכן היא נבדקת בנפרד.
  const rsiVolatility = text.split(/[.;]/).some(
    sen => /RSI/.test(sen) && /תנודתיו/.test(sen));
  if (rsiVolatility) fail('איכות', 'RSI קושר לתנודתיות — טעות מקצועית');
  else ok('RSI לא קושר לתנודתיות');
}
